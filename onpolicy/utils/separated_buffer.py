...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space, n_agents):
        ...
        self.value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.n_agents),
            dtype=np.float32,
        )
        ...
        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, log_act_shape),
            dtype=np.float32,
        )
        self.rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, self.n_agents),
            dtype=np.float32,
        )
