...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.advantages = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, num_agents),
            dtype=np.float32,
        )
    ...
    def insert(
        ...
        self.rewards[self.step] = np.tile(
            rewards.squeeze(-1), self.rewards.shape[-1]
        ).reshape(*self.rewards.shape[1:])
    ...
    def feed_forward_generator(
        ...
        for indices in sampler:
            ...
            agent_ids_batch = agent_ids[indices]
            if advantages is None:
                adv_targ = None
            else:
                adv_targ = advantages[indices]

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, agent_ids_batch
