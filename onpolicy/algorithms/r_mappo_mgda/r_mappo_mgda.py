    ...
    def __init__(self, args, policy, device=torch.device("cpu")):
        ...
        if self._use_popart:
            ...
            self.value_normalizer = ValueNorm(
                self.policy.num_agents, device=self.device
            ).to(self.device)
        else:
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        for a in range(n_agents):
            ...
            policy_loss = self.ppo_loss(
                a, imp_weights, adv_targ, active_masks_batch, dist_entropy
            )
        ...
        if not self.use_mgda:
            filter_grad_indx = [
                i for i in range(len(policy_grads)) if gradnorm[i] > self.mgda_eps
            ]
        ...
        sol, _ = MinNormSolver.find_min_norm_element(
            [policy_grads[t] for t in filter_grad_indx]
        )
        ...
        other = {"min_grad_norm": np.min(gradnorm), "max_grad_norm": np.max(gradnorm)}
