...
class GridworldRunner(Runner):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                for agent_id in range(self.num_agents):
                    train_infos.update(
                        {
                            f"agent{agent_id}/average_episode_rewards": np.mean(
                                self.buffer.rewards[..., agent_id]
                            )
                            * self.episode_length
                        }
                    )
    ...
    def warmup(self):
        ...
        self.buffer.agent_ids[0] = np.tile(
            np.arange(self.num_agents), self.n_rollout_threads
        ).reshape(self.buffer.agent_ids[0].shape)
